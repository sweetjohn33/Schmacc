...
def start_game(game):
    ...
    for player in game.return_players():
        for i in range(len(player.lands())):
            rand_index = randint(0, len(game.return_basic_creatures()) - 1)
            player.add_creature(game.return_basic_creatures()[rand_index])

            del game.return_basic_creatures()[rand_index]

            rand_index2 = randint(0, len(game.return_buildings()) - 1)
            player.add_building(game.return_buildings()[rand_index2])

            del game.return_buildings()[rand_index2]
    ...
    for player in game.return_players():
        ...
        for a in range(len(player.creatures())):
            game.put_monster_on_land(player, player.creatures()[a])

        print("\nLet's place " + player.name() + "'s buildings!")

        for a in range(len(player.buildings())):
            game.put_building_on_land_to_start(player, player.buildings()[a])
    ...
    turn_counter = 0
    turn_player = game.return_players()[turn_counter % len(game.return_players())]
    game.building_phase(turn_player, turn_counter)

    def building_phase(game, turn_player, turn_counter):
        """
        ...
        while g != 6:
            ...
            if g == 1:
                ...
                if h == 1:
                    game.buy_something_random("Spell", 2, turn_player, game.return_spells())
                    h = 6

                elif h == 2:
                    if game.check_slots(turn_player, "Building"):
                        game.buy_somthing_specific("Building", 10, turn_player, game.return_buildings())
                    else:
                    ...
                    if game.check_slots(turn_player, "Creature"):
                        game.buy_something_random("Creature", 5, turn_player, game.return_basic_creatures())
                    else:
                    ...
                    if game.check_slots(turn_player, "Creature"):
                        game.buy_something_random("Creature", 8, turn_player, game.return_normal_creatures())
                    else:
                ...
                if h == 5:
                    if game.check_slots(turn_player, "Creature"):
                        game.buy_something_random("Creature", 14, turn_player, game.return_elite_creatures())
                    else:
                        print("\nYou dont have a free monster slot!")
                    ...
                    if land_index != 999:
                        ...
                        if len(land.building_slots()) != 0:
                            ...
                            if building_sold != 1000:
                                ...
                                game.return_buildings().append(land.building_slots()[building_sold])
                ...
                if answer == 2:
                    ...
                    if len(turn_player.spells()) >= 3:
                        for j in range(3):
                            ...
                            game._Spells_to_sell.append(turn_player.spells()[item_sold])
                        ...
                        while bool(choice.isdigit()) == 0 or int(choice) not in [1, 2]:
                            if tried_before > 0:
                                print("Invalid input. Try again!\n")
                            print(game._Spells_to_sell + "\n1) Yes\n2) No\n")
                            choice = input("Are you sure you want to sell these cards??")
                            tried_before += 1
                        if int(choice) == 1:
                            for gorf in game._Spells_to_sell:
                                game.return_spells().append(gorf)
                                del gorf
                ...
                if land_index != 999:
                    ...
                    if len(land.monster_slot()) != 0:
                        ...
                        if int(monster_sold) == 1:
                            game.tribute_monster(turn_player, land)
                            g = ""
                            tried_before = 0
                            while bool(g.isdigit()) == 0 or int(g) not in range(1, 7):
                                if tried_before > 0:
        ...
        game.main_phase(turn_player, 1, turn_counter)

    def main_phase(game, turn_player, phase_number, turn_counter):
    ...
    def battle_phase(game, turn_player,turn_counter):
        ...
        game.main_phase(turn_player, 2, turn_counter)